...
***Note:*** we use the `#` symbol to make comments. The text following the character is ignored by the code interpreter allowing someone to add information in plain language to guide other users, or even themselves at a later date. [This is a very good practice, use it!!]{.underline}
...
For more complex objects, such as single-cell and spatial transcriptomic data, researchers will simply save their R object as it is using commands such as `save` or `saveRDS`. The advantage of doing this is that is maintains all of the information and formatting that the data generator will have added which does not effectively transfer to a table. For example; *Did the analysis use dose as a number or a factor? Was the data stored as a matrix, dataframe, or vector?* Providing the R object can help improve the reproducibility of analyses in certain scenarios.

Here we will focus on R objects which represent single-cell RNA sequencing datasets. These datasets typically are stored using a complex format which, in a very simplified description, allows the use of `slots` to hold metadata, raw counts, normalized counts, gene details, dimension reductions, and more...
...
As previously mentioned, objects can be saved using `save` or `saveRDS`. In combination with the load function, this maintains the R object as it was when it was produced.

Using `save`:

```{r}
# Save seurat_object to outputs
save(seurat_object, file = './outputs/exported_using_save.rds')

# Import the file into your environment using original variable name
load('./outputs/exported_using_save.rds') 
```
...
```{r}
# Save seurat_object to outputs
saveRDS(seurat_object, file = './outputs/exported_using_save.rds')

# Import the file into your environment using a new variable name
saved_seurat_object = readRDS('./outputs/exported_using_save.rds')

# Show that the figure is exactly the same
DimPlot(saved_seurat_object, label = TRUE, label.size = 5) + NoLegend()
```
