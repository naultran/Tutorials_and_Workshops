...
```{r}
# Load the package dependencies for this workbook
...
```{r}
# Load the PBMC dataset which was previously downloaded
# We use the filtered_gene_bc_matrices which has removed empty droplets
pbmc.data <- Read10X(data.dir = "./downloads/filtered_gene_bc_matrices/hg19/")

# Initialize the Seurat object with the raw (non-normalized data)
# When setting up the object we remove items which are clearly not cells (min.features) or expressed genes (min.cells)
...
```{r}
# Let's look at what is in the object
print(seurat_object)
```
...
# We make sure to preserve the gene names
rownames(raw_counts) = rownames(seurat_object)

# We also make sure to preserve the cell barcodes
...
```{r}
# Examine the seurat object again
print(seurat_object)
```
...
```{r}
# Count the number of columns and print them
print(paste0('Cells: ', ncol(raw_counts)))

# Count the number of rows and print them
